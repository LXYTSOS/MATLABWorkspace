...
aa = rand(2,n_samp/2);%创建一个2×n_samp/2的随机矩阵，rand函数产生由在(0, 1)之间均匀分布的随机数组成的数组
...
x     = radius.*cos(theta);
y     = radius.*sin(theta);
label = 1*ones(1,length(x));  % label for Class 1，ones是生成一个单位矩阵
...
shuffle_seq = randperm(n_col);%randperm随机打乱一个数字序列。
